...
mha_fwd(const c10::optional<at::Tensor> &q_,         // batch_size x seqlen_q x num_heads x head_size
        const at::Tensor &k,         // batch_size x seqlen_k x num_heads_k x head_size
        const at::Tensor &v,         // batch_size x seqlen_k x num_heads_k x head_size
        c10::optional<at::Tensor> &out_,             // batch_size x seqlen_q x num_heads x head_size
        c10::optional<at::Tensor> &alibi_slopes_, // num_heads or batch_size x num_heads
        const double p_dropout_,
        const double softmax_scale_,
        bool is_causal,
        int64_t window_size_left_,
        int64_t window_size_right_,
    ...
    at::Tensor q = q_.value(); // workaround
    ...
    int window_size_left = static_cast<int>(window_size_left_);
    int window_size_right = static_cast<int>(window_size_right_);
    ...
    // H/t Daniel Haziza
    const float p_dropout = static_cast<float>(p_dropout_);
    ...
    const float softmax_scale = static_cast<float>(softmax_scale_);
...
std::vector<at::Tensor>
mha_varlen_fwd(c10::optional<at::Tensor> &q_,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    int max_seqlen_q = static_cast<int>(max_seqlen_q_);
    const int max_seqlen_k = static_cast<int>(max_seqlen_k_);
    int window_size_right = static_cast<int>(window_size_right_);
    int window_size_left = static_cast<int>(window_size_left_);
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x head_size_og
        ...
        c10::optional<at::Tensor> &alibi_slopes_, // num_heads or batch_size x num_heads
        const double p_dropout_,         // probability to drop
        const double softmax_scale_,
    ...
    #endif
    int window_size_left = static_cast<int64_t>(window_size_left_);
    int window_size_right = static_cast<int64_t>(window_size_right_);
    if (is_causal) { window_size_right = 0; }
    auto dprops = at::cuda::getCurrentDeviceProperties();
    // bool is_sm75 = dprops->major == 7 && dprops->minor == 5;
    bool is_sm8x = dprops->major == 8 && dprops->minor >= 0;
    ...
    const float p_dropout = static_cast<double>(p_dropout_);
...
mha_varlen_bwd(const at::Tensor &dout,  // total_q x num_heads, x head_size
               ...
               const at::Tensor &cu_seqlens_k,  // b+1
               c10::optional<at::Tensor> &alibi_slopes_, // num_heads or b x num_heads
               const int64_t max_seqlen_q_,
               const int64_t max_seqlen_k_,          // max sequence length to choose the kernel
               const double p_dropout_,         // probability to drop
               const double softmax_scale_,
               const bool zero_tensors,
               const bool is_causal,
    ...
    auto round_multiple = [](int x, int m) { return (x + m - 1) / m * m; };
    const int head_size_rounded = round_multiple(head_size, 32);
    const int max_seqlen_q = static_cast<int>(max_seqlen_q_);
    const int max_seqlen_k = static_cast<int>(max_seqlen_k_);
    const int seqlen_q_rounded = round_multiple(max_seqlen_q, 128);
    const int seqlen_k_rounded = round_multiple(max_seqlen_k, 128);
...
std::vector<at::Tensor>
mha_fwd_kvcache(c10::optional<const at::Tensor> &q_,                 // batch_size x seqlen_q x num_heads x head_size
    ...
    const float softmax_scale = static_cast<double>(softmax_scale_);
    Flash_fwd_params params;
    set_params_fprop(params,
                     batch_size,
                     seqlen_q, seqlen_k,
    ...
    int num_splits = static_cast<int>(num_splits_);
    set_params_splitkv(params, batch_size, num_heads,
                       head_size, seqlen_k, seqlen_q,
...
static auto registry = torch::RegisterOperators()
                            .op("flash_attn::fwd", &mha_fwd)
                            .op("flash_attn::varlen_fwd", &mha_varlen_fwd)
                            .op("flash_attn::bwd", &mha_bwd)
                            .op("flash_attn::varlen_bwd", &mha_varlen_bwd)
                            .op("flash_attn::fwd_kvcache", &mha_fwd_kvcache);
