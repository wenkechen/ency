...
def _flash_attn_varlen_forward(
    ...
    maybe_contiguous = lambda x: x.contiguous() if x.stride(-1) != 1 else x
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    out, q, k, v, out_padded, softmax_lse, S_dmask, rng_state = torch.ops.flash_attn.varlen_fwd(
        q,
        k,
        v,
        None,
...
def _flash_attn_varlen_backward(
    ...
    (
        ...
        dv,
        softmax_d,
    ) = torch.ops.flash_attn.varlen_bwd(
        dout,
        q,
        k,
        v,
...
def flash_attn_with_kvcache(
    ...
    out, softmax_lse = torch.ops.flash_attn.fwd_kvcache(
        q,
        k_cache,
        v_cache,
        k,
